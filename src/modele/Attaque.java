...
public class Attaque extends Thread implements Global {
	...
	public void run() {
		...
		if (victime!=null && !victime.estMort() && victime.getNumEquipe()!=attaquant.getNumEquipe()) {
			// son du joueur touché
			if (victime.getNumEquipe() == 1) {
				switch(victime.getNumPerso()) {
				case 1 : jeuServeur.envoi(MONSTERGHURT);break;
				case 2 : jeuServeur.envoi(MONSTERSHURT);break;
				case 3 : jeuServeur.envoi(MONSTERDHURT);break;
				case 4 : jeuServeur.envoi(MONSTERSAHURT);break;
			};
			}else {
				jeuServeur.envoi(KNIGHTHURT);				
			}
			...
			if (victime.estMort()) {
				...
				if (victime.getNumEquipe() == 1) {
					switch(victime.getNumPerso()) {
						case 1 : jeuServeur.envoi(MONSTERGDEATH);break;
						case 2 : jeuServeur.envoi(MONSTERSDEATH);break;
						case 3 : jeuServeur.envoi(MONSTERDDEATH);break;
						case 4 : jeuServeur.envoi(MONSTERSADEATH);break;
					};
				}else {
					jeuServeur.envoi(KNIGHTDEATH);				
				}

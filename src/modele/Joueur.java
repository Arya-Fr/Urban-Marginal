...
public class Joueur extends Objet implements Global {
	...
	public void affiche(String etat, int etape) {
		...
		if (numEquipe == 1) {
			label.getjLabel().setIcon(new ImageIcon(MONSTER+numPerso+etat+etape+"d"+orientation+EXTIMAGE));
		}else {
			label.getjLabel().setIcon(new ImageIcon(KNIGHT+numPerso+etat+etape+"d"+orientation+EXTIMAGE));
		}
	 ...
	 * @param numPerso
	 */
	public void initPerso(String pseudo, int numPerso,int numEquipe, Hashtable<Connection, Joueur> lesJoueurs, ArrayList<Mur> lesMurs) {
		this.pseudo = pseudo ;
		...
		switch (action) {
			...
			case TIRE :   
				if (!boule.getLabel().getjLabel().isVisible()) {
					if (numEquipe == 1) {
						switch(numPerso) {
							case 1 : jeuServeur.envoi(MONSTERGFIGHT);break;
							case 2 : jeuServeur.envoi(MONSTERSFIGHT);break;
							case 3 : jeuServeur.envoi(MONSTERDFIGHT);break;
							case 4 : jeuServeur.envoi(MONSTERSAFIGHT);break;
						};
					}else {
						jeuServeur.envoi(KNIGHTFIGHT);						
					}
